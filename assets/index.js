...
removeButton.addEventListener('click', function() {
  imagesContainer.removeChild(mealContainer);
});
...
function getMealDetails(meal) {
  ...
  const image = `<img src="${meal.strMealThumb}" alt="${meal.strMeal}" style="max-width: 200px; max-height: 200px; border: none; border-radius: 10px; display: flex; justify-content: center; align-items: center;" />`
...
//         currentMealDetail.style.display = 'none';  
//       }
//       currentMealDetail = ingredientDiv;  
//       showModal(getIngredientsList)(meal)       
//       showModal(ingredientDiv.innerHTML)
//       showModal(getMealDetails(meal));
//     })
...
//   }
// const steps = meal.strInstructions.split('\n').map(step => `<p>${step}</p>`).join('');
// const image = `<img src="${meal.strMealThumb}" alt="${meal.strMeal}" />`;
